export const AUTHOR = {
  name: "Fuji",
  pseudonym: "Waltahh",
  shortName: "Perkenalkan, saya Fuji. Di internet biasa dipanggil waltahh atau terserah yang mau manggil apa.",
  handle: "Blog ini tempat saya mencatat hal-hal yang menarik. Sesukanya, semaunya, seadanya. Bisa teknologi, anime, atau sekadar renungan.",
  tagline: "Jikalau bermanfaat, alhamdulillah 🙂. Kalau tidak, tidak apa-apa.",
  bio: "Selamat membaca.",
  bioLong: "Halo 👋\n\nPerkenalkan, saya Fuji. Di internet biasa dipanggil waltahh atau terserah yang mau manggil apa.\n\nBlog ini tempat saya mencatat hal-hal yang menarik. Sesukanya, semaunya, seadanya. Bisa teknologi, anime, atau sekadar renungan.\n\nJikalau bermanfaat, alhamdulillah 🙂. Kalau tidak, tidak apa-apa.\n\nSelamat membaca.",
  url: "https://x.com/fuhara16",
} as const;